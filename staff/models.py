...
class Nurse(Person):
    person = models.OneToOneField(User, related_name='nurses', null=True, on_delete=models.SET_NULL)
    avatar = models.ImageField(upload_to='avatars/nurses', null=True, blank=True)
    specialty = models.ManyToManyField(Speciality)
    user_type = models.ForeignKey(UserType, related_name='nurses', null=True, on_delete=models.SET_NULL)
...
class OtherStaff(Person):
    person = models.OneToOneField(User, null=True, on_delete=models.SET_NULL)
